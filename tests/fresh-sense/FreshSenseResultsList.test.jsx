...
describe('FreshSenseResultsList', () => {
  ...
  it('renderiza el encabezado y la lista de elementos cuando hay resultados', () => {
    ...
    sampleResults.forEach((r) => {
      expect(
        screen.getByText(new RegExp(`${r.emoji}\\s+${r.item}\\s+–\\s+${r.assessment}`, 'i'))
      ).toBeInTheDocument();
    });
  });

  it('cumple con roles/accessibilidad básicos: el encabezado es detectado por getByRole', () => {
    const sampleResults = [{ item: 'Manzana', assessment: 'madura', emoji: '🍎' }];
    render(<FreshSenseResultsList results={sampleResults} />);

    // getByRole('heading') ya valida accesibilidad mínima (por ejemplo, <h3>)
    const heading = screen.getByRole('heading', { level: 3 });
    expect(heading).toHaveTextContent(/ingredientes/i);
  });
});
